...
def main():
    ...
    TrainDataLoader = makeTorchDataLoader(TrainDataset, batch_size= 2)
    TestDataLoader = makeTorchDataLoader(TestDataset, batch_size = 2)
    ...
    try:
        for epoch in range(2):
            ...
            for batch_index, batch_dict in enumerate(TrainDataLoader):
                ...
                acc_t = computeAccuracy(logits, batch_dict[3])
            ...
            for batch_index, batch_dict in enumerate(TestDataLoader):
                model.eval()
                batch_dict = tuple(t.to(device) for t in batch_dict)
                outputs = model(input_ids=batch_dict[0], 
                    token_type_ids = batch_dict[1],
                    attention_mask = batch_dict[2],
                    labels=batch_dict[3])
                loss, logits = outputs[:2]
